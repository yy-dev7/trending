...
6. 保存为 `news/YYYY-MM-DD.html`（今天的日期），并更新两处索引：
   - `news/index.html`：在 `<!-- LATEST -->` 标记下方加一条指向今天这份的链接（最新在最上方）。
   - `index.html`（首页）：把 `data-latest="news"` 那个 `<a>` 的 `href`
     改成 `news/YYYY-MM-DD.html`（即首页「阅读今日」直达当天动态）。
7. 提交改动并**直接推送到 `main` 分支**（`git push origin main`），提交信息：「行业动态 + 日期」。
